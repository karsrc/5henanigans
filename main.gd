...
# Hook up the new clickable buttons
...
func _ready():
	# Ensure the mouse is visible and usable!
	...
	# Connect the buttons for mouse clicks
...
func cycle_character(direction: int):
	...
	if is_instance_valid(char_sprite):
		var tw = create_tween()
		char_sprite.modulate = Color(10, 10, 10, 1) # Flash white
...
func update_character_display():
	...
	if roster[current_idx] == "RYOMEN":
		char_sprite.self_modulate = Color("#df84a5") # Your palette pink
	else:
